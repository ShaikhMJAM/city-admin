...
export const getConfirmationGridData = async ({ screenFlag, compCode }) => {
  ...
  if (screenFlag === "userLimit") {
    ...
    APIURL = "GETMERCHONBOARDGRID";
    parameters = { COMP_CD: compCode, CONFIRMED: "N" };
  } else if (screenFlag === "virtualCard") {
    APIURL = "GETVIRTUALCARDIMAGELIST";
    parameters = { CONFIRMED: "N" };
  } else if (screenFlag === "webRerouting") {
    APIURL = "GETWEBREROUTINGGRID";
    parameters = { CONFIRMED: "N" };
  }
