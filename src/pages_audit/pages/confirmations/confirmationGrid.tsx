...
import { MerchantOnboardingGridMetadata } from "../operations/merchantOnboarding/gridmetadata";
import { MerchantViewDetails } from "../operations/merchantOnboarding/viewdetails/merchantDetails";
import { CardImageFormWrapper } from "../configurations/virtualCardImage/virtualCardImageForm";
import { VirtualCardImageUploadGridMetaData } from "./metaData/virtualCardImageUpload";
import { WebReroutingConfigGridMetaData } from "../configurations/webRerouting/gridMetadata";
import { WebReroutingFormWrapper } from "../configurations/webRerouting/webReroutingForm";
...
export const Confirmations = ({ screenFlag }) => {
  ...
  } else if (screenFlag === "merchantOnboard") {
    gridMetaData = MerchantOnboardingGridMetadata;
    gridMetaData.gridConfig.gridLabel = "Merchant Onboarding Confirmation";
  } else if (screenFlag === "virtualCard") {
    gridMetaData = VirtualCardImageUploadGridMetaData;
  } else if (screenFlag === "webRerouting") {
    gridMetaData = WebReroutingConfigGridMetaData;
    gridMetaData.gridConfig.gridLabel = "Web Rerouting Confirmation";
  }
            ...
            element={
              screenFlag === "userLimit" ? (
                ...
                <OperatorMstConfirmDetails
                  ...
                  defaultmode={"view"}
                />
              ) : screenFlag === "merchantOnboard" ? (
                <MerchantViewDetails
                  isDataChangedRef={isRefreshRef}
                  closeDialog={ClosedEventCall}
                  formView={"confirm"}
                />
              ) : screenFlag === "virtualCard" ? (
                <CardImageFormWrapper
                  ...
                  defaultmode={"confirm"}
                />
              ) : screenFlag === "webRerouting" ? (
                <WebReroutingFormWrapper
                  handleDialogClose={ClosedEventCall}
                  isDataChangedRef={isRefreshRef}
                  moduleType={"webRerouting"}
                  defaultmode={"confirm"}
                />
              ) : (
                <></>
...
const ConfirmationGridWrapper = ({ screenFlag }) => {
  return (
    <ClearCacheProvider>
      <Confirmations
...
export default ConfirmationGridWrapper;
