...
import { PGWConfirmGridMetaData } from "./gridMetadata";
...
const PGWConfigConfirm = () => {
  ...
  const { data, isLoading, isFetching, isError, error, refetch } = useQuery<
    any,
    any
  >(["getPGWGridData", "N", "CONFIRM"], () =>
    API.getPGWGridData("N", "CONFIRM")
  );
  ...
  useEffect(() => {
    return () => {
      queryClient.removeQueries(["getPGWGridData", "N", "CONFIRM"]);
    };
  }, [getEntries]);
  // PGWConfirmGridMetaData.gridConfig.gridLabel = "PGW Merchant Users Confirmation";
  // PGWConfirmGridMetaData
  const onAcceptPopupYes = (rows) => {
    result.mutate({
      confirmed: "Y",
      tranCode: rows[0]?.data?.TRAN_CD ?? "",
      srCode: rows[0]?.data?.SR_CD ?? "",
    });
  };
  ...
  return (
    <Fragment>
      ...
      <GridWrapper
        key={`PGWConfirmGrid`}
        finalMetaData={PGWConfirmGridMetaData as GridMetaDataType}
        data={data ?? []}
        setData={() => null}
...
export default PGWConfigConfirm;
