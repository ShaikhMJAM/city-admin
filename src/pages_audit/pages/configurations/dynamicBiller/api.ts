...
export const getLanguageCode = () => GeneralAPI.GetMiscValue("LANGUAGE_TYPE");
...
export const updateBillerChargeConfig = async (formData: any) => {
  const { status, message, messageDetails } = await AuthSDK.internalFetcher(
...
export const getDynBillerScreenNotes = async (
  categoryID,
  subCategoryID,
  billerID
) => {
  const { data, status, message, messageDetails } =
    await AuthSDK.internalFetcher("GETDYNBILLSCREENNOTEDATA", {
      CATEGORY_ID: categoryID,
      SUB_CATEGORY_ID: subCategoryID,
      BILLER_ID: billerID,
    });
  if (status === "0") {
    return { screenNotes: data };
  } else {
    throw DefaultErrorObject(message, messageDetails);
  }
};

export const updateDynamicBillerScreenNotes = async (formData: any) => {
  const { status, message, messageDetails } = await AuthSDK.internalFetcher(
    "DYNBILLERSCREENNOTEDML",
    formData
  );
  if (status === "0") {
    return message;
