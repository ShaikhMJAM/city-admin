...
import * as APIDTL from "./servMobileValidMstForm/api";
import { ServMobileValidGridMetaData } from "./gridMetadata";
import { useSnackbar } from "notistack";
import { ServMobileValidMstWrapper } from "./servMobileValidMstForm";
...
export const ServMobilValid = () => {
  ...
  const { data, isLoading, isFetching, isError, error, refetch } = useQuery<
    any,
    any
  >(["getServMobilValidGridData", "ALL", "ENTRY"], () =>
    API.getServMobileValidGridData("ALL", "ENTRY")
  );
  ServMobileValidGridMetaData.gridConfig.gridLabel =
    "Service wise Mobile Number Validation Template Mapping";

  const deleteMutation = useMutation(
    updateAUTHDetailDataWrapperFn(APIDTL.updServMobileValidFormData),
    {
      onError: (error: any) => {},
      onSuccess: (data) => {
  ...
  useEffect(() => {
    return () => {
      ...
      queryClient.removeQueries(["getServMobilValidGridData", "ALL", "ENTRY"]);
    };
  }, [getEntries]);
  ...
  return (
    <Fragment>
      ...
      <GridWrapper
        key={`ServMobilValidGrid`}
        finalMetaData={ServMobileValidGridMetaData as GridMetaDataType}
        data={data ?? []}
        setData={() => null}
...
export const ServMobileValidGridWrapper = () => {
  return (
    <ClearCacheProvider>
      <ServMobilValid />
    </ClearCacheProvider>
...
// const DeleteServMobilValid = ({ closeDialog, isDataChangedRef, rows }) => {
//   const { enqueueSnackbar } = useSnackbar();
