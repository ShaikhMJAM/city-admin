...
import * as APIDTL from "./msMessageConfigForm/api";
import { MsMessageConfigGridMetaData } from "./gridMetadata";
import { useSnackbar } from "notistack";
import { MsMessageConfigFormWrapper } from "./msMessageConfigForm";
import { MessageBoxWrapper } from "components/custom/messageBoxWrapper";
import { MsMessageUpdateServices } from "./updateServices";
...
export const MsMessageConfig = () => {
  ...
  const { data, isLoading, isFetching, isError, error, refetch } = useQuery<
    any,
    any
  >(["getMsMessageConfigGridData"], () => API.getMsMessageConfigGridData());
  const deleteMutation = useMutation(
    updateAUTHDetailDataWrapperFn(APIDTL.updMsMessageConfigFormData),
    {
      onError: (error: any) => {},
      onSuccess: (data) => {
  ...
  useEffect(() => {
    return () => {
      ...
      queryClient.removeQueries(["getMsMessageConfigGridData"]);
    };
  }, [getEntries]);
  ...
  return (
    <Fragment>
      ...
      <GridWrapper
        key={`MsMessageConfigGrid`}
        finalMetaData={MsMessageConfigGridMetaData as GridMetaDataType}
        data={data ?? []}
        setData={() => null}
      ...
      <Routes>
        ...
        <Route
          ...
          element={
            <MsMessageUpdateServices ClosedEventCall={ClosedEventCall} />
          }
        />
      </Routes>
      {isDelete ? (
...
export const MsMessageConfigGridWrapper = () => {
  return (
    <ClearCacheProvider>
      <MsMessageConfig />
    </ClearCacheProvider>
