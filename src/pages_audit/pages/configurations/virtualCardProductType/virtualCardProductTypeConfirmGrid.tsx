...
const VirtualCardProductTypeConfirm = () => {
  ...
  useEffect(() => {
    return () => {
      ...
      queryClient.removeQueries(["getvCardProdTypeGridData", "N"]);
    };
  }, [getEntries]);

  const { refetch, data, isLoading, isFetching, isError, error } = useQuery(
    ["getvCardProdTypeGridData", "N"],
    () => API.getvCardProdTypeGridData("N")
  );
  ...
  // VirtualCardProdTypeGridMetaData.gridConfig.allowRowSelection = false;
  return (
    <Fragment>
      {isError === true ? (
        <Alert
      ...
      <GridWrapper
        key={`vCardProdTypeConfirmGrid`}
        //@ts-ignore
        finalMetaData={VirtualCardProdTypeGridMetaData}
        data={data ?? []}
        setData={() => null}
        actions={actions}
...
export default VirtualCardProductTypeConfirm;
