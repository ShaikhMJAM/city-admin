...
const ViewEditChargeTempMaster: FC<{
  ...
  const mutationConfirm = useMutation(API.updServiceChargeConfigConfirm, {
    onError: (error: any) => {
      let errorMsg = "Unknown Error occured";
      if (typeof error === "object") {
        errorMsg = error?.error_msg ?? errorMsg;
  ...
  if (formMode === "confirm") {
    chargeTempMasterDetailsMetaData.masterForm.form.label =
      "Service Charge Template Confirmation";
  } else {
    chargeTempMasterDetailsMetaData.masterForm.form.label =
      "Service Charge Template Configuration";
  }
  metadata = cloneDeep(
