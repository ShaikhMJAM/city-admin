import { FC, useEffect, useState, useRef, useContext, useMemo } from "react";
...
import { Grid, Tooltip, Typography, Button, Dialog } from "@mui/material";
...
const MFSConfigEntry: FC<{
  ...
  defaultmode: string;
  gridData?: any;
}> = ({ isDataChangedRef, closeDialog, defaultmode, gridData }) => {
  ...
  const onSubmitHandler: SubmitFnType = (
    ...
    data = {
      ...
      TRN_LABEL_BN: StringtoUnicode(data?.TRN_LABEL_BN ?? "").replaceAll(
        "\\u",
        "\\"
      ),
    };
    ...
    let oldData = {
      ...rows?.[0]?.data,
      TRN_LABEL_BN: StringtoUnicode(
        rows?.[0]?.data?.TRN_LABEL_BN ?? ""
      ).replaceAll("\\u", "\\"),
    };
  ...
  const masterMetadata: MetaDataType = useMemo(
    () => extractMetaData(MFSConfigEntryMetaData, formMode),
    [MFSConfigEntryMetaData, formMode]
  ) as MetaDataType;
      ...
      <Grid container>
        <Grid item xs={12} sm={12} md={12}>
          <FormWrapper
            ...
            initialValues={
              formMode === "add" ? {} : (rows?.[0]?.data as InitialValuesType)
            }
