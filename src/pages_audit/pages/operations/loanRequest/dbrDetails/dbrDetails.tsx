...
const DBRDetailViewEdit: FC<{
  ...
  const lockUnlockMutation = useMutation(API_LOAN.setLockUnlockRequest, {
    onError: (error: any) => {
      isOpenRef.current = "ERROR";
      closeDialog();
  ...
  const onSubmitHandler: SubmitFnType = (
    ...
    if (actionFlag === "View") {
      ...
      if (
        result?.data?.[0]?.CONFIRMED === "D" ||
        result?.data?.[0]?.CONFIRMED === "Y" ||
        result?.data?.[0]?.CONFIRMED === "R"
      ) {
        enqueueSnackbar("Request has already been processed.", {
          variant: "warning",
        });
      } else {
  ...
  useEffect(() => {
    const handleBeforeUnload = (event) => {
      lockUnlockMutation.mutate({ tranCd: tran_cd, type: "R" });
    };
    window.addEventListener("beforeunload", handleBeforeUnload);
    return () => {
      if (isOpenRef.current === "OPEN") {
        lockUnlockMutation.mutate({ tranCd: tran_cd, type: "C" });
      } else if (isOpenRef.current === "RELEASE") {
        lockUnlockMutation.mutate({ tranCd: tran_cd, type: "R" });
      }
      isOpenRef.current = "RELEASE";
