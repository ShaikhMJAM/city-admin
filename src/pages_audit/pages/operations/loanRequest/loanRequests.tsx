...
export const LoanRequest = ({ screenFlag }) => {
  ...
  const selectedDates = (retrievalValues, dateObj) => {
    setOpen(false);
    if (dateObj) retrievalParaRef.current = dateObj;
    result.mutate({
      fromDate:
  ...
  useEffect(() => {
    ...
    // });
    result.mutate({
      fromDate: retrievalParaRef.current?.A_FROM_DT
        ? format(retrievalParaRef.current?.A_FROM_DT, "dd/MM/yyyy")
        : format(new Date(), "dd/MM/yyyy"),
      toDate: retrievalParaRef.current?.A_TO_DT
        ? format(retrievalParaRef.current?.A_TO_DT, "dd/MM/yyyy")
        : format(new Date(), "dd/MM/yyyy"),
      screenID: screenFlag,
    });
    return () => {
      ...
      <Fragment>
        ...
        {open ? (
          <DateRetrievalDialog
            ...
            defaultData={retrievalParaRef.current}
          />
        ) : null}
      </Fragment>
    </StrictMode>
