...
export const MasterDetailsForm = forwardRef<any, MasterDetailsArgumentType>(
  (
    ...
    const isDetailsHideRef = useRef(false);
    ...
    useEffect(() => {
      isDetailsHideRef.current = isDetailHide;
      if (isDetailHide && Boolean(serverError)) {
        setServerError("");
      }
    }, [isDetailHide, setServerError]);
    ...
    const handleSubmitDetails = async (
      ...
      if (isDetailsHideRef.current) {
        ...
        let data = {
          ...
          ...resultValueObj,
          DETAILS_DATA: { isNewRow: [], isUpdatedRow: [], isDeleteRow: [] },
          ...updateValue,
        };
        onSubmitData({
          data,
        ...
        if (hasError === true) {
          ...
          if (result.length === 0) {
            ...
            let editModeValidateRow = result.filter((one) =>
              Boolean(one?._hidden)
            );
            if (
              displayMode === "edit" &&
              editModeValidateRow.length === result.length
            ) {
              endSubmit(true);
              setServerError("Atleast one row must be in detail.");
              return;
        ...
        >
          ...
          <div
            style={{
              paddingRight: "10px",
              paddingLeft: "10px",
              display: isDetailHide ? "none" : "inherit",
            }}
          >
            <GridWrapper
